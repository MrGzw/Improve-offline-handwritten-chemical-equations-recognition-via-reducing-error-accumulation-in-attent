...
def test(test_loader, model, tools, current_epoch=0):  # 尝试修改处1    test函数用在两处，一是模型训练完进行预测，而是训练过程中，每个epoch都进行验证时使用
    ...
    for sample_batched in test_loader:
        ...
        output, out_length = model[2](features[-1], A, target, length, current_epoch, True)  # 尝试修改处2
        tools[2].add_iter(output, out_length, length, label, current_epoch)
    tools[2].show()
...
if __name__ == '__main__':
    ...
    # --------------------------------
    # prepare tools
    train_acc_counter = Attention_AR_counter('train accuracy: ', cfgs.dataset_cfgs['dict_dir'], cfgs.dataset_cfgs['case_sensitive'], state = "train")
    test_acc_counter = Attention_AR_counter('\ntest accuracy: ', cfgs.dataset_cfgs['dict_dir'], cfgs.dataset_cfgs['case_sensitive'], state = "test")
    ...
    for nEpoch in range(0, cfgs.global_cfgs['epoch']):
        for batch_idx, sample_batched in enumerate(train_loader):
            ...
            features = model[0](data)           
            A = model[1](features)
            output, attention_maps = model[2](features[-1], A, target, length, nEpoch)  # model = (model_fe, model_cam, model_dtd)   此处可修改DTD为计划采样 尝试修改处3
            # output, attention_maps = model[2](features[-1], A, target, length)
            # computing accuracy and loss 
            train_acc_counter.add_iter(output, length.long(), length, label, nEpoch)
