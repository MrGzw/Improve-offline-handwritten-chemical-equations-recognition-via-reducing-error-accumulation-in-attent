...
global_cfgs = {
    ...
    'epoch': 51,
    'show_interval': 60,
    'test_interval': 600  #训练集每隔多少对所有测试集进行测试
}

dataset_cfgs = {
    'dataset_train': IAMDataset,   #IAMSynthesisDataset,
    'dataset_train_args': {
        # 'img_list': 'E:/Guzhiwen_project/data/IAM/train_list.txt',
        'img_list': 'E:/Guzhiwen_project/Hw_chemical_formula/train_list.txt',   #'data/IAM/train_list.txt',
        'img_height': 192,
        'img_width': 2048,
        # 'augment': False, # with the data augmentation toolkit   选择IAMSynthesisDataset时加上
    },
    'dataloader_train': {
        'batch_size': 24,
        'shuffle': True,
    ...
    'dataset_test': IAMDataset,
    'dataset_test_args': {
        # 'img_list': 'E:/Guzhiwen_project/data/IAM/eval_list.txt',
        'img_list': 'E:/Guzhiwen_project/Hw_chemical_formula/eval_list.txt',
        'img_height': 192,
        'img_width': 2048,
    },
    'dataloader_test': {
...
net_cfgs = {
    ...
    'DTD': DTD,
    'DTD_args': {
        'nclass': 73, # extra 2 classes for Unkonwn and End-token
        'nchannel': 256,
        'dropout': 0.7,
    },
...
saving_cfgs = {
    'saving_iter_interval': 600,
    'saving_epoch_interval': 15,
...
def mkdir(path_):
    ...
    for i in range(0, len(paths) - 1):
        command_str = command_str + paths[i] + '/'
    command_str = command_str[0:-1]
    os.system(command_str)    #提前转化成系统指令，创建文件路径
